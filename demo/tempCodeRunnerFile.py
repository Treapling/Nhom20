...
def load_model_from_disk() -> Optional[object]:
    ...
    1) results/lightgbm.pkl
    2) results/random_forest.pkl
    3) results/xgboost.pkl
    Nếu không có, trả về None.
    """
    candidates = [
        os.path.join("results", "lightgbm.pkl"),
        os.path.join("results", "random_forest.pkl"),
        os.path.join("results", "xgboost.pkl"),
    ]
    last_exc = None
    for p in candidates:
...
alley_width = st.sidebar.number_input(
    "Độ rộng hẻm (m) — nhập 0 nếu nhà mặt tiền", min_value=0.0, max_value=12.0, value=4.0, step=0.5
)
...
if model is None:
    st.sidebar.error(
        "Không tìm thấy file mô hình trong `results/`\n"
...
with col1:
    ...
    li_exists = os.path.exists(os.path.join('results','lightgbm.pkl'))
    rf_exists = os.path.exists(os.path.join('results','random_forest.pkl'))
    xg_exists = os.path.exists(os.path.join('results','xgboost.pkl'))
    st.write("- LightGBM model file:", "Yes" if li_exists else "No")
    st.write("- RandomForest model file:", "Yes" if rf_exists else "No")
    st.write("- XGBoost model file:", "Yes" if xg_exists else "No")
...
with col2:
    ...
    st.info(
        "Demo mang tính chất thử nghiệm trong phạm vi đại học, chưa đủ tin cậy cho việc áp dụng thực tế."
    )
...
if predict_button:
    ...
    if model is None:
        ...
        try:
            ...
            used_model_path = 'results/lightgbm.pkl' if os.path.exists(os.path.join('results','lightgbm.pkl')) else 'results/random_forest.pkl' if os.path.exists(os.path.join('results','random_forest.pkl')) else 'results/xgboost.pkl' if os.path.exists(os.path.join('results','xgboost.pkl')) else 'Unknown model'
        except Exception as e_primary:
            # Primary model failed; try loading RF explicitly and predict
            try:
                rf_path = os.path.join('results', 'random_forest.pkl')
